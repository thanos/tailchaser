...
def test_backfill():
    ...
    def gx():
        while True:
            record = yield ()
            open(os.path.join(tmp_dir, os.path.basename(record[0])), 'ab').write(record[2])

    consumer = gx()
    consumer.send(None)
    source_pattern = os.path.join(TEST_PATH, 'logs', 'opendirectoryd.*')
    main([__name__, '--only-backfill', '--clear-checkpoint', source_pattern], consumer)
    for src_file_path in glob.glob(source_pattern):
        dst_file_path = os.path.join(tmp_dir, str(Tailer.make_sig(src_file_path)))
