...
class Tailer(Producer):
    ...
    def should_tail(self, file_to_check, checkpoint):
        if self.args.verbose:
            print 'should_tail', file_to_check, checkpoint
        ...
        if sig == checkpoint[0] and checkpoint[2] < stat.st_size:
            ...
            if self.args.verbose:
                print 'SIG the same', retval
            return retval
        if stat.st_mtime > checkpoint[1]:
            if self.args.verbose:
                print " Younger", file_to_check, (sig, stat.st_mtime, 0)
    ...
    def load_checkpoint(self, checkpoint_filename):
        try:
            ...
            if self.args.verbose:
                print 'loading', checkpoint_filename, (sig, mtime, offset)
    ...
    def save_checkpoint(self, checkpoint_filename, checkpoint):
        if self.args.verbose:
            print 'dumping', checkpoint_filename, checkpoint
        ...
        while True:
            try:
                ...
                to_tail = filter(None, sorted((self.should_tail(file_to_check, checkpoint)
                                 for file_to_check in glob.glob(self.args.source_pattern)), key=lambda x: x[1][1] if x else x))
                if self.args.verbose:
                    print 'checkpoint', checkpoint
                if self.args.verbose:
                    print 'to_tail', to_tail
                if self.args.verbose:
                    time.sleep(5)
